...
class ControlBotModule():
    ...
    def __init__(self):
        ...
        # Setting imported libaries available outside '__init__' scope but inside 'ControlBotModule' scope (Imports only when the class is instanced and let them available to the others class' methods):
...
class VisionBotModule():
    ...
    def display_text(self, frame, text, position, thickness, font_size=1, text_color=(255, 255, 255), shadow_color=(128, 128, 128), font_style=cv2.FONT_HERSHEY_SIMPLEX, line_style=cv2.LINE_AA):
...
class SupportBotModule():
    ...
    def angular_coefficient(self, point1, point2, decimals=0, to_degrees=False):
        ...
        if to_degrees == False:
            ...
            return round(degrees(atan(m)), decimals)
